...
namespace FuguFirecracker.TakeNote
{
    public static class TaskMaster
    {
        ...
        {
            GUI.skin.settings.cursorColor = EditorGUIUtility.isProSkin ? Color.white : Color.black;
            var backgroundColorCached = GUI.backgroundColor;

            if (task.IsColored)
            {
                GUI.backgroundColor = task.DrawColor;
                GUI.contentColor = task.TextColor;
            }
            else
            {
            ...
            if (task.DoShowMore)
            {
                ...
                EditorGUILayout.LabelField(string.Format("Created on :     {0}", task.CreationDate), Style.Mini);
                if (task.IsCompleted)
                {
                ...
                {
                    if (e.shift)  UtilityMinder.instance.LaunchAsUtility(task);
                    else PopupWindow.Show(task.ClickOnRect, new EditTaskPopUp(task));
                }
            ...
            GUI.contentColor = Main.Window.ResetColor;
        }
        ...
        public static void Assimilate(Task tempTask, Task taskAtHand)
        {
            ...
            taskAtHand.TextColor = tempTask.TextColor;
        }
