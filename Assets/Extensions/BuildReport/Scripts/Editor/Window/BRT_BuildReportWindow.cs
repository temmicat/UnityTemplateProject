...
public class BRT_BuildReportWindow : EditorWindow
{
	...
	/// <summary>
	/// The Asset Dependencies generationData being used
	...
	void _OpenBuildInfo(string filepath)
	{
		...
		{
			Debug.LogError(string.Format("Build Report Tool: Invalid generationData in build info file: {0}", filepath));
		}
		...
		if (System.IO.File.Exists(unityBuildReportFilePath))
		{
			...
			{
				Debug.LogWarning(string.Format("Can't open additional build info generationData due to Unity version incompatibility.\n\n{0}", e));
				_unityBuildReport = null;
			}
		}
		else
